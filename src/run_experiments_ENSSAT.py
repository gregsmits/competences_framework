...
#Analysis of the university program ENSSAT Alternance (RNCP35781) and associated ROME skills
university_program = "ENSSAT"
RNCPCODE = "RNCP35781"
rncp_file = "data/fiches_rncp/RNCP35781.json"

#Associated ROME FILES
rome_files = [ "data/fiches_rome/ROME_M1802.json",  "data/fiches_rome/ROME_M1804.json",  "data/fiches_rome/ROME_M1805.json",  "data/fiches_rome/ROME_M1806.json"]


#Associated JOB OFFERS
job_offers_directories = ["data/offres_emploi_JSON_enssat/2022", "data/offres_emploi_JSON_enssat/2023", "data/offres_emploi_JSON_enssat/2024","data/offres_emploi_JSON_enssat/2025"]

#########################################
# Creation of the three skills-bases
#########################################

#RNCP SKILLS BASE FIP training (RNCP35781)
...
skills_bases_job_offers = dict( )
for job_offers_directory in job_offers_directories:

    skills_bases_job_offers[job_offers_directory] = skillloader.load_base_from_directory(job_offers_directory)
...
#########################################
pbs_indexer_name = "RNCP38637_problems_family.idx"
resources_indexer_name = "RNCP38637_resources.idx"
...
for sb, name in tqdm.tqdm([ (skills_base_rome, "ROME skills base")] + [(skills_bases_job_offers[dir], f"Job Offers skills base {dir}") for dir in skills_bases_job_offers]):
...
for fil in skills_bases_job_offers:
    skills_base_job_offers = skills_bases_job_offers[fil]
...
weighted_resources_job_offers = dict()
for fil in skills_bases_job_offers:
    skills_base_job_offers = skills_bases_job_offers[fil]
    weighted_resources_job_offers[fil] = skills_base_job_offers.assign_resource_degrees()
...
for fil in weighted_resources_job_offers:
    print(f"\n\nComparing ranked resources between ROME skills base and Job offers skills base for directory {fil}:")
    print(f"\t- Number of resources in common: {len(set(weighted_resources_rome.keys()).intersection(set(weighted_resources_job_offers[fil].keys())))} out of {len(weighted_resources_rome)} for ROME and {len(weighted_resources_job_offers[fil])} for Job offers.")
    spearman_rome_joboffers = spearman_correlation(weighted_resources_rome, weighted_resources_job_offers[fil])
    print(f"\t- Spearman correlation: {spearman_rome_joboffers}")
    print("\t- Main differences :")
    differences_rome_joboffers = raw_differences(weighted_resources_rome, weighted_resources_job_offers[fil])
